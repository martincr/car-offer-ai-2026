...
export default function Page({ params }: { params: { id: string } }) {
    ...
    return (
      <main className="mx-auto max-w-md px-4 pb-10 pt-10">
        <Card className="p-5">
          ...
          <div className="mt-1 text-sm text-zinc-600">This link may be expired (prototype store resets on restart).</div>
          <div className="mt-4">
  ...
  const d = lead.data;
  const carTitle = `${d.vehicle?.year ?? "—"} ${d.vehicle?.make ?? ""} ${d.vehicle?.model ?? ""}`.trim();
      ...
      <Card className="p-5">
        ...
        <div className="mt-3 grid gap-2 text-sm">
          ...
          {(d.existingOffer?.source ?? "None") !== "None" ? (
            <>
              <div className="mt-2 border-t border-zinc-200/70 pt-3 text-sm font-medium text-zinc-900">Existing offer</div>
              <div className="flex items-start justify-between gap-3">
              ...
              <div className="mt-2 text-xs text-zinc-500">Dealers should verify. Do not accept edited screenshots.</div>
          ...
          {d.notes ? (
            <>
              <div className="mt-2 border-t border-zinc-200/70 pt-3 text-sm font-medium text-zinc-900">Notes</div>
              <div className="rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-700">{d.notes}</div>
            </>
          ) : null}
        </div>
      </Card>
          ...
          {d.photos?.length ? (
            <div className="mt-3 grid grid-cols-2 gap-2">
              {d.photos.map((src, idx) => (
                <a key={idx} href={src} target="_blank" rel="noreferrer" className="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50">
              ...
              {lead.bids.map((b) => (
                <div key={b.id} className="rounded-2xl border border-zinc-200 bg-white p-3">
                  <div className="flex items-center justify-between">
                    <div className="font-medium">{b.action === "pass" ? "Pass" : `Bid: $${b.amount}`}</div>
