  ...
  return (
    <Card className="p-5">
      ...
      <div className="mt-4 grid gap-3">
        <Input label="Dealer name (optional)" placeholder="ABC Motors" value={dealerName} onChange={(e) => setDealerName(e.target.value)} />
        <Input label="Your cash offer" placeholder="e.g. 26,500" value={amount} onChange={(e) => setAmount(e.target.value)} inputMode="numeric" />
        <Textarea label="Message to seller (optional)" placeholder="Pickup timing, inspection, etc." value={message} onChange={(e) => setMessage(e.target.value)} />

        <div className="flex gap-2">
          <Button className="flex-1" size="lg" disabled={!canBid} onClick={() => send("bid")}>
            {loading ? "Sending…" : "Send bid"}
          </Button>
          <Button variant="secondary" className="flex-1" size="lg" disabled={loading} onClick={() => send("pass")}>
            Pass
          </Button>
        </div>

        {status ? (
          <div className={`rounded-2xl border p-4 text-sm ${status.ok ? "border-zinc-200 bg-zinc-50 text-zinc-700" : "border-red-200 bg-red-50 text-red-700"}`}>
            {status.text}
          </div>
        ) : null}
