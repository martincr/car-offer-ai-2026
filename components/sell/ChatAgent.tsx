    ...
    try {
      ...
      if (stage === 'vin') {
        ...
        if (!isValidVin(vin)) {
          push(
            'assistant',
            'That VIN doesn’t look right. It should be 17 characters (letters & numbers). Try again.'
          );
          return;
        }
        ...
        <div className="space-y-3">
          ...
          {stage === 'offerUpload' ? (
            <div className="mt-2 rounded-3xl border border-zinc-200 bg-white p-4">
              ...
              <div className="mt-1 text-xs text-zinc-500">
                PDF or screenshot. Don’t edit it — dealers verify.
              </div>
              <input
          ...
          {showSubmit ? (
            <div className="mt-2 rounded-3xl border border-zinc-200 bg-white p-4">
              ...
              <div className="mt-1 text-xs text-zinc-500">
                We’ll syndicate this to dealers and text you bids.
              </div>
              ...
              <div className="mt-3 grid gap-2 text-xs text-zinc-600">
                ...
                <div>
                  <span className="text-zinc-500">Car:</span> {draft.vehicle?.year ?? '—'}{' '}
                  {draft.vehicle?.make ?? ''} {draft.vehicle?.model ?? ''}{' '}
                  {draft.vehicle?.trim ? `(${draft.vehicle.trim})` : ''}
                </div>
                <div>
                  <span className="text-zinc-500">Contact:</span> {draft.contact.name ?? '—'} •{' '}
                  {draft.contact.phone ?? '—'}
                </div>
                <div>
